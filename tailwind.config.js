...
function getElevationStyles() {
  ...
  for (let i = 0; i < 24; ++i) {
    const umbra = umbras[i];
    const penumbra = penumbras[i];
    const ambient = ambients[i];
    elevationStyles[`z${i+1}`] = `${umbra} ${umbraColor}, ${penumbra} ${penumbraColor}, ${ambient} ${ambientColor}`;
  }
